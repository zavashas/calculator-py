...
def game():
    ...
    print(f"\n{person['Имя']}: \"Вот и закончился учебный год\"\n")

    choice = igrok_choice("\"Свобода! Каникулы!\"",
                         ['Выглянуть в окно', 'Посмотреть в зеркало'])
    input_choice(choice, person)

    print(f"\n{person['Имя']}: \"Поверить не могу, что не нужно будет рано вставать. Высплюсь, наконец..\"\n")

    print("Настя: \"Кто как. Это ты все сдала. А у меня еще хвосты.\"\n")

    print(f"{person['Имя']}: \"Беда. Но диплом МПТ за красивые глаза не дают.\"\n")
    ...
    if choice == 1:
        print(f"{person['Имя']}: \"Почему бы и не пойти? Попрощаться с одногруппниками надо. Правда ведь?\"\n")
        print("Настя: \"Да. Многие уедут из общаги.\"\n")
    else:
        print(f"{person['Имя']}: \"Даже не знаю.. В прошлый раз было скучно. Вряд ли теперь будет иначе\"\n")
    ...
    print(f"Эдик: \"Вообще-то я по делу, {person['Имя']}, тебе посылка.\"\n")

    print(f"{person['Имя']}: \"От кого?\"\n")

    print("Эдик: \"Без понятия.\"\n")

    print(f"{person['Имя']} взяла посылку и отошла к окну.\n")

    choice = igrok_choice(f"{person['Имя']}: \"Странно..\"",
    ...
    print(f"{person['Имя']}: \"Интересно, кто прислал?\"\n")

    print(
        "...\nНебольшая коробка была завернута в обычную почтовую бумагу. Но ни штемпелей, ни имени отправителя не было.\n")

    print(f"{person['Имя']} разорвала бумагу и открыла коробку.\n")

    print("Внутри была открытка и маска.\n")

    print(
        f"{person['Имя']}: \"Приглашение на маскарад.. вампиров?! Что за чушь? Форма одежды любая. Вход при наличии маски.. И адрес.\"\n")

    print("Настя: \"Имя, что там? От кого?\"\n")

    print(f"{person['Имя']}: \"Посмотри сама\"\n")

    print("Настя: \"Забавно.. Но это же сегодня вечером!\"\n")

    choice = igrok_choice(
         f"{person['Имя']}:\"Да, думаю съездить туда.\"\n  Эдик: \"Стоп, стоп! {person['Имя']}, ты решила слиться? Оставайся с нами. Тебе многие будут рады.\"\n ...\n Парень обнял {person['Имя']} за плечи,\n Подсказка: Некоторые решения меняют характер {person['Имя']}, \n Одни повышают её  👊Стойкость - силу духа, твёрдость характера. \n Другие 📚 Дипломатию -умение договориться, обаяние и рассудительность. \n Выбирайте то, что подходит именно вам.",
    ...
    if choice == 1:
        print(f"\nВечером {person['Имя']} пошла на вечеринку со своими одногруппниками и хорошо провела время.")
    else:
        print(
            f"\nВечером {person['Имя']} вызвала такси и поехала по указанному в письме адресу. По приезде её встретил высокий мужчина в черной маске и ударил по голове. Name отключилась.")
